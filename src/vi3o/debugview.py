...
class DebugViewer(object):
    ...
    def _stack(self, image_array):
        if max(len(img.shape) for img in image_array) == 3:
            images = [
                img if len(img.shape) == 3 else np.stack([img, img, img], 2)
    ...
    def _view_image_array(self):
        ...
        self.color_mask = np.hstack(
            [[len(ii.shape) == 3] * ii.shape[1] for _, ii in self.image_array]
        )
        ...
        self.image = pyglet.image.ImageData(
            img.shape[1], img.shape[0], f, img.tostring(), pitch
        )
    ...
    def on_mouse_motion(self, x, y, dx=None, dy=None):
        ...
        y = (
            self.image.height
            - int((y - self.offset[1] - self.scroll[1]) / self.scale)
            - 1
        )
    ...
    def on_resize(self, winw, winh):
        if self.image:
            ...
            self.scale = sc = (
                min(
                    float(winw) / self.image.width, float(winh - lh) / self.image.height
                )
                * self.zoom
            )
            w, h = int(sc * self.image.width), int(sc * self.image.height)
            self.offset = (
                (self.window.width - w) / 2,
                (self.window.height - lh - h) / 2 + lh,
            )
