...
class Mkv(object):
    def __init__(self, filename, grey=False, reindex=False):
        ...
        if need_index or reindex:
            ...
            self.frame.sort()
            self.systime_offset = iter(
                self
            ).estimate_systime_offset()  # FIXME: This makes a second pass over the file parsing it
...
class MjpgDecoder(object):
    ...
    def decode_frame(self, frm, pixels, pts, grey):
        ...
        if grey:
            r = mjpg_lib.mjpg_open_buffer(
                m, frm.data, frm.len, mjpg_lib.IMTYPE_GRAY, mjpg_lib.IMORDER_INTERLEAVED
            )
        else:
            r = mjpg_lib.mjpg_open_buffer(
                m, frm.data, frm.len, mjpg_lib.IMTYPE_RGB, mjpg_lib.IMORDER_INTERLEAVED
            )
        if r != mjpg_lib.OK:
...
class MkvIter(object):
    ...
    def next(self):
        ...
        while True:
            r = self.decoder.decode_frame(
                self.frm, pixels, self.pts, self.channels == 1
            )
            if r >= 0:
